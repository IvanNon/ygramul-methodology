...
This is the working reference for the eleven levers Ygramul organizes its work around. Each lever covers a category of work that contributes to organic visibility on classical search and on AI-search surfaces. Each one has its own diagnostic signals and failure modes.
...
Senior technical SEOs don't usually carry eleven levers in their head. They think in roughly four buckets: *can the bot find it, can the bot index it, will it rank, did it work?*

The eleven levers are a more granular version of that same intuition:

- **Find and index** — Levers 1, 2, 3 (Crawl Architecture, Canonical Architecture, Page Technical Signals)
- **Rank** — Levers 4, 5, 6, 7, 8, 9 (Keyword Strategy, Content, Programmatic, Internal Linking, Authority/Trust/GEO, Local SEO & GBP)
- **Measure and learn** — Levers 10 and 11 (Brand Demand, Auto-Research)

The granularity is the point. Senior practitioners can compress these eleven back to four when they want to think fast. The eleven exist so that audit findings have a clean home and so the work scales beyond what one person can hold in their head.

### Three lever groupings

Not all eleven levers run on the same timescale or in the same way:

- **Engagement levers (1–9)** — sequenced work done during a client engagement. Run in order. Each unblocks the next.
- **Long-arc lever (10)** — brand and search demand work, which operates on multi-year timescales and runs in parallel to engagement work, not sequenced after it.
- **Cross-engagement lever (11)** — auto-research and outcome measurement. Operates across the full client base and feeds back to update the rules used in Levers 1–10.
...
- **Public relations and demand-generation execution.** Ygramul measures branded search outcomes (Lever 10). It does not pitch journalists, run podcast outreach, or execute PR strategy.
- **Multi-location franchise SEO at enterprise scale (50+ locations).** Local visibility for small-to-medium businesses is in scope under Lever 9. Multi-location franchise programs at enterprise scale are their own discipline with dedicated specialists; Ygramul is not the right tool for that scale.
...
This document describes the framework as designed. Some components operate today across active engagements. Others are partially built and being instrumented. Where a lever's chapter describes capabilities still being developed — particularly Lever 11's auto-research loop — the chapter says so plainly.
...
The work of building visibility for place-bound businesses — businesses where the customer's search intent includes a geographic component and where the result needs to appear in the map pack, the local pack, and on the business's Google Business Profile. Lever 9 covers GBP optimization, local citations, review management, local landing pages, and local schema.

### What success looks like

The business appears in the map pack for its primary commercial queries within its service area. The Google Business Profile is complete, current, and actively maintained — services, attributes, hours, photos, posts, products. Reviews accumulate at a credible cadence and are responded to with care. Citations on Yelp, Apple Maps, Bing Places, and relevant industry directories are accurate and consistent. NAP (name, address, phone) is identical across every surface. Local landing pages — city pages, service-area pages, neighborhood pages — are differentiated, useful, and link cleanly into the rest of the site.

### Signal categories

- Google Business Profile completeness (services, attributes, hours, photos, posts, products, Q&A)
...
- Map pack and Local Pack ranking signals
- Inbound links from locally relevant sources

### Why this lever sits at position 9

Local SEO depends on most upstream levers being solved. The site's technical foundation (Levers 1–3) must be sound — a local business with a broken canonical strategy or unindexable pages is invisible regardless of GBP work. The content asset library (Levers 5–6) must include local-relevant content. The authority and entity infrastructure (Lever 8) provides the substrate that local schema and GBP signals build on. Running local SEO work before these upstream levers are solved produces visible map pack presence on a shaky foundation; the upstream issues eventually pull the rankings down.

### Pattern observed in practice

Local SEO work for a solo professional in a competitive urban service category produced more visibility lift than expected, faster than expected — because the competition wasn't doing the basics. Most competing professionals had GBPs that were created once and never revisited. Photos were old or absent. Services were undeclared. Reviews were neglected. The fix wasn't sophisticated: complete the GBP, add Service entries with descriptions, post weekly updates, request reviews systematically, write three differentiated service-area landing pages with proper local schema, and audit citations for NAP consistency. Within 60 days, the business was ranking in the local pack for primary commercial queries it had been invisible for. The pattern: in many local service categories, the competitive bar is low and the basics are doing 80% of the work.

### Common failure modes

- **GBP treated as profile creation rather than ongoing optimization.** The profile gets set up once. Photos go up. Hours are entered. Then it sits for two years while competitors actively post weekly updates, request reviews, and adjust services. The neglected profile loses ground.
- **Citations built once, never audited.** A business changes its phone number or moves locations. The website updates. Half the citation graph doesn't. NAP inconsistency spreads silently and weakens local ranking signals over time.
- **Reviews ignored as a ranking signal.** Reviews matter for both visibility and conversion. Businesses that don't request reviews systematically, don't respond to negative reviews professionally, and don't surface reviews on their site lose ground in both ranking and trust.
- **Local landing pages treated as duplicate content.** A business with five service areas creates five pages with the same content and the city name swapped. Search engines treat these as low-value duplicates. Differentiation — local content, local imagery, local context — is the work.
- **Local schema missing or incomplete.** LocalBusiness schema with no Service entries, no GeoCoordinates, no OpeningHoursSpecification. The schema validates but provides minimal signal.

---

## Lever 10 — Brand & Search Demand Generation

The long-arc work of growing demand for the brand itself — branded search volume, share of voice for category terms, brand mention frequency, and the demand-side keyword universe that feeds future ranking opportunities. Lever 10 operates on multi-year timescales and runs in parallel to engagement work, not sequenced after it.
...
This is why Lever 10 sits in parallel rather than sequenced last. Effective engagements start brand demand measurement on day one, not at month 12. The work compounds quietly while engagement levers do their faster-feedback work.
...
The cross-engagement work of measuring what shipped, scoring its outcomes, and feeding the resulting patterns back to update the rules used in Levers 1 through 10. Lever 11 is the framework's learning loop. It operates continuously across the full client base and is what allows Ygramul to sharpen with use rather than going stale.
...
- Cross-vertical pattern detection (do findings that work in SaaS also work in publishing? in commerce? in local? where do they not?)
...
Most real findings cross levers. A canonical loop on a programmatic page is both a Lever 2 (Canonical Architecture) issue and a Lever 6 (Programmatic SEO) issue. A schema gap on author pages is both a Lever 3 (Page Technical Signals) issue and a Lever 8 (Authority, Trust & GEO Visibility) issue. A NAP inconsistency between the site and external citations is both a Lever 8 (organizational entity infrastructure) issue and a Lever 9 (Local SEO) issue.
