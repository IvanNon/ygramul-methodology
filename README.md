...
A working framework for technical SEO and GEO (Generative Engine Optimization) — organized around eleven levers, grounded in a primary-source Codex, and designed to learn from each engagement so the rules sharpen with use rather than going stale.
...
Ygramul is a personalized data-driven framework designed to learn which optimizations actually move the needle for a specific site. Recommendations are tied to measurable hypotheses. Hypotheses are designed to be tested against outcomes, and the tests are designed to update the rules that drive the next round of audits.

- **Eleven levers** — nine sequenced engagement levers, one long-arc brand demand lever, and one cross-engagement auto-research lever. Documented in [LEVERS.md](./LEVERS.md).
- **A primary-source Codex** — curated reference library of leaks, patents, papers, and antitrust testimony, used as a RAG layer for audits, briefs, and recommendations
- **An auto-research loop** — four-step cycle (baseline → implementation → evaluation → update), designed to refine the rules from each engagement
...
Ygramul is structured as a system instead of a deliverable. Findings are tied to hypotheses. Hypotheses are designed to be tested against measured outcomes. Outcomes are designed to update the rules that drive the next round. The version of the methodology applied to a new client should become more accurate over time as cumulative measurement updates the framework's priors.
...
Each lever is a category of work that contributes to organic visibility, with its own diagnostic signals and failure modes.

The engagement levers are sequenced. Optimizing for AI citation before fixing crawl waste is wasted effort, because the citation signals can't reach pages the crawler isn't indexing reliably. Running programmatic SEO before fixing canonical architecture multiplies the problem rather than solving it. The order matters.

Two levers operate outside the engagement sequence. Lever 10 (Brand & Search Demand Generation) runs in parallel on a multi-year clock. Lever 11 (Auto-Research & Outcome Measurement) operates across the full client base and feeds back to update the rules used in Levers 1–10.

Full chapter-by-chapter detail in [LEVERS.md](./LEVERS.md).
...
- **[Schema Detector](https://github.com/IvanNon/schema-detector-skill)** — single-URL structured data audit, addresses Lever 3 (Page Technical Signals)
- **[Ygramul AIO Cascade](https://github.com/IvanNon/ygramul-aio-cascade)** — AI Overview opener block generation through 14 specialized editor-judges, addresses Lever 5 (Content Production & Optimization)
...
This repo is the canonical source for the Ygramul methodology — the philosophy, the eleven levers, the relationship between human judgment and AI execution. It does not contain the implementation playbooks, the agent prompts, or the rejection-criteria framework that paying clients receive.

If you're a senior SEO or GEO practitioner, [LEVERS.md](./LEVERS.md) is where the substance lives. If you're trying to recreate Ygramul from this repo alone, you'll get the architecture but not the engine.
